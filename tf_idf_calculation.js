...
for(var i=0; i<x; i++)
{
    const string =  problem_desc[i];
    let array = f.filter(string);
    array.forEach(element => {
...
// var itfValue = [];
// setArray.forEach(element => {
//     var val = Math.log(1228/string_doc_presence_count.get(element));
//     itfValue.push(val);
//     // console.log(val);
// });
...
// fs.writeFile('itf_value.txt', content, err => {
//   if (err) {
//     console.error(err)
//     return
//   }else{
...
// fs.writeFile('tf_value_doc.txt', content, err => {
//     if (err) {
//       console.error(err)
//       return
//     }else{
...
fs.writeFile('all_keywords.txt', content, err => {
    if (err) {
      console.error(err)
      return
    }else{
